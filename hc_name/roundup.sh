...
do 
  ...
  set_field 1 "older_match" <good_name_redo_p8.txt | grep -v "^older_match$" >>redo_fname_$run.txt
# the grep -v gets rid of blank lines. yuck   20151211 hib
  ...
  good_name_redo_p*.txt singleton_name_p1*.txt singleton_name_p2*.txt random_seed_f run_$run
  cp redo_lname_$run.txt run_$run
  cp redo_fname_$run.txt run_$run
